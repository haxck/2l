...
export default function Home() {
    ...
    function newCommemoration(title, time) {
      setCommemorations([...commemorations, { title, time }])
      localStorage.setItem("commemorations",JSON.stringify([...commemorations,{ title, time }]))
    }
    function delCommemoration(index) {
      let c= JSON.parse(localStorage.getItem("commemorations")).toReversed()
      c.splice(index,1)
      setCommemorations([...c])
